...
export async function POST(request: Request) {
  try {
    ...
    const { name, type, keywords, sentiment, daysOfWeek, startTime, endTime, replyTemplate, action, priority } = await request.json();
    ...
    if (type === 'keyword' && (!keywords || keywords.length === 0)) {
      return NextResponse.json({ error: 'Keywords are required for keyword rules' }, { status: 400 });
    }

    if (type === 'sentiment' && !sentiment) {
      return NextResponse.json({ error: 'Sentiment is required for sentiment rules' }, { status: 400 });
    }

    if (type === 'time' && (!daysOfWeek || !startTime || !endTime)) {
      return NextResponse.json({ error: 'Days and time range are required for time rules' }, { status: 400 });
    }
